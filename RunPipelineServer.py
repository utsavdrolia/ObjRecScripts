...
import subprocess
from CreateDBNearby import create
from FileSystemGlobals import *

# To run EvaluateClientServer
...
def launch_pipeline(server, querypath, dbpath, resultpath, featuretype, matchertype_db, matchertype_cache, matchersize):
    subprocess.call([JAVA, JAVA_MAX_MEM, OPENCVLIB, CLASSPATH_STR, CLASSPATH, OR_SERVER, querypath, dbpath, resultpath,
                     PARS[featuretype], PARS_DB[featuretype], featuretype, matchertype_db, MATCHER_PARS_DB[matchertype_db],
                     matchertype_cache, MATCHER_PARS[matchertype_cache], matchersize, server])


def run_pipeline(server, indbpath, outdbpath, num, inqpath, outqpath, descnum, resultspath, featuretype, matchertype_db, matchertype_cache, matchersize):
    dbfile, qfile = create(indbpath, outdbpath, num, inqpath, outqpath)
    change_num_desc(descnum, PARS[featuretype])
    launch_pipeline(server, qfile, dbfile, resultspath, featuretype, matchertype_db, matchertype_cache, str(matchersize))
