...
if __name__ == '__main__':
    ...
    cache = PerfectLFUCache(0)

    for cache_size in cache_sizes:
        cache.setmax(cache_size)
        print ("Size:{}".format(cache_size))
        init_misses = cache.getmisses()
        for i in range(1000):
            q = z.next()
            v = cache[q]
            queries.append(q)
        ...
        curr_ratio = float(cache.getmisses() - init_misses) / 1000
        print ("Miss Ratio:{}".format(ratio))
        ratios.append((ratio, 0))
        lat.append((cache_lat + L * curr_ratio, 0))
